...
| R6 | Delete buttons on portfolio rows are hidden by default and only visible in Manage mode |
| R7 | Manage mode reveals delete buttons on portfolio rows, shows the trash section with deleted portfolios, and changes the Manage button to "Done" to exit |
...
| UC-PORTFOLIO-001-S10 | P2 | ❌ | Given user views the portfolio list, When not in Manage mode, Then delete buttons are hidden on portfolio rows and trash section is not visible | R6 |
| UC-PORTFOLIO-001-S11 | P2 | ❌ | Given user enters Manage mode, When viewing the portfolio list, Then delete buttons appear on portfolio rows, trash section with deleted portfolios is visible, and Manage button changes to Done | R7 |
