...
| R3 | Bearer token takes priority over CF_Authorization cookie and CF header if both present |
...
| UC-AUTH-002-S03 | P0 | ✅ | Given request has both Bearer token and CF header (different users), When calling any authenticated endpoint, Then use Bearer token user | R3 |
