...
async function fetchCashMovements(): Promise<{
  ...
  const res = await worker.fetch(`http://localhost/api/portfolios/${portfolioId}/cash-movements`, {
    headers: authHeaders(),
  });
...
describe("GET /api/portfolios/:id/cash-movements", () => {
  ...
  it("returns 404 for non-existent portfolio", async () => {
    const res = await worker.fetch(`http://localhost/api/portfolios/99999/cash-movements`, {
      headers: authHeaders(),
    });
